...
@Component({
  ...
  template: `
     <div class="min-h-[100dvh] flex flex-col items-center justify-end p-4 bg-black relative overflow-hidden">
       
       @if (isMpinScreen()) {
         <!-- Top Box for MPIN Screens -->
         <div class="max-w-md w-full bg-white p-6 pt-10 rounded-none border-2 border-black relative flex flex-col items-center justify-center mb-4">
            <button (click)="goBack()" class="absolute top-4 left-4 text-black font-bold flex items-center hover:underline z-10">
               <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
       ...
       <!-- Bottom/Main Box -->
       <div class="max-w-md w-full bg-white p-8 rounded-none border-2 border-black relative overflow-hidden flex flex-col">
