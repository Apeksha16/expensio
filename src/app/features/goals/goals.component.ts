  ...
  template: `
    <div class="h-full bg-white p-4 flex flex-col gap-4">
      @if (goalService.isLoading()) {
        ...
        <!-- Top Summary Box -->
        <div class="bg-black text-white p-5 rounded-2xl flex flex-col gap-1 relative overflow-hidden shadow-[6px_6px_0px_0px_rgba(244,63,94,1)] mt-2">
        ...
        <div class="flex-1 flex flex-col gap-4 pb-28 mt-2">
          @if (activeGoals().length > 0) {
            @for (goal of activeGoals(); track goal.id) {
              <button
                ...
                class="w-full bg-white border-2 border-black rounded-2xl p-4 flex flex-col gap-3 text-left shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[5px_5px_0px_0px_rgba(0,0,0,1)] transition-all active:scale-[0.99]"
              >
                <div class="flex items-start gap-4 w-full">
                  <div class="w-12 h-12 bg-goals-primary border-2 border-black text-white flex items-center justify-center shrink-0 rounded-xl">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                      <path [attr.d]="getGoalIconPath(goal.icon)"></path>
                    </svg>
                  </div>
                  <div class="flex flex-col gap-0.5 flex-1 min-w-0 pr-2">
                    <span class="font-black text-lg text-black truncate">{{ goal.name }}</span>
                ...
                <div class="w-full flex flex-col gap-1 mt-1">
                  <div class="flex justify-between text-[10px] font-black text-black uppercase tracking-widest">
                    <span>₹{{ goal.saved_amount | number: '1.0-0' }} Saved</span>
                    <span>₹{{ goal.total_amount | number: '1.0-0' }} Goal</span>
                  </div>
                  <div class="h-3 w-full bg-white border-2 border-black rounded-full overflow-hidden">
                    <div
                      class="h-full bg-goals-primary transition-all duration-500 origin-left animate-[fillProgress_1s_ease-out] border-r-2 border-black"
                      [style.width.%]="getProgress(goal)"
                    ></div>
                  </div>
                </div>

                <div class="flex justify-between items-center w-full gap-2 mt-2 pt-3 border-t-2 border-black border-dashed">
                  @if (paidGoalsThisMonth().has(goal.id)) {
                    <span class="text-[10px] font-black text-emerald-600 uppercase tracking-widest flex items-center gap-1 border border-emerald-200 bg-emerald-50 px-2 py-1 rounded">
                    ...
                    <span class="text-[10px] font-black text-red-600 uppercase tracking-widest border border-red-200 bg-red-50 px-2 py-1 rounded">
                      Due · ₹{{ goal.calculated_installment | number: '1.0-0' }}
                    </span>
                  } @else {
                    <span class="text-[10px] font-black text-gray-500 uppercase tracking-widest bg-gray-100 px-2 py-1 rounded border border-gray-200">
                      No Payment Due
                    </span>
                  }
                  <button
                    (click)="addFunds($event, goal)"
                    class="bg-black text-white border-2 border-black px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-[2px_2px_0px_0px_rgba(242,106,141,1)] hover:shadow-[3px_3px_0px_0px_rgba(242,106,141,1)] active:scale-[0.95] transition-all flex items-center gap-2"
                  >
                    Add Funds
                  </button>
                </div>
            ...
            <div class="flex-1 flex flex-col items-center justify-center p-8 text-center h-[300px]">
              ...
              <p class="text-black font-extrabold text-xl">No active goals</p>
          ...
          @if (archivedGoals().length > 0) {
            <div class="mt-6 mb-2">
              <h3 class="font-black text-black uppercase tracking-widest text-sm px-1">
                Archived Goals
              </h3>
            </div>
            @for (goal of archivedGoals(); track goal.id) {
              <div class="w-full bg-white border-2 border-black rounded-2xl p-4 flex flex-col gap-3 text-left opacity-75">
                <div class="flex justify-between items-center w-full pb-3 border-b-2 border-black border-dashed">
                ...
                <div class="flex items-start gap-4">
                  <div class="w-12 h-12 bg-gray-100 text-gray-500 border-2 border-gray-300 flex items-center justify-center shrink-0 rounded-xl">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                      <path [attr.d]="getGoalIconPath(goal.icon)"></path>
                    </svg>
                  </div>
                  <div class="flex flex-col gap-0.5 flex-1 min-w-0 pr-2">
                    <span class="font-black text-lg text-black truncate">{{ goal.name }}</span>
                    <span class="text-[10px] font-black text-gray-500 uppercase tracking-widest">Achieved</span>
                  </div>
                  <div class="flex flex-col items-end shrink-0">
                    <span class="font-black text-lg text-black">₹{{ goal.total_amount | number: '1.0-0' }}</span>
