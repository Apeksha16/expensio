        ...
        @if (goalService.isLoading() || expenseService.isLoading()) {
          <div class="flex flex-col gap-3">
            @for (i of [1, 2, 3]; track i) {
              <div class="w-full bg-gray-50 border border-gray-100 rounded-2xl h-[72px] animate-pulse"></div>
            }
          </div>
        } @else {
          <div class="mb-6 bg-white border border-gray-100 text-gray-900 p-6 relative overflow-hidden rounded-2xl shadow-sm">
            <div class="flex flex-col gap-1 relative z-10">
              ...
              <span class="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mt-1">
                Goal: ₹{{ $safeNavigationMigration(goal()?.total_amount) | number: '1.0-0' }}
              </span>
            </div>

            <div class="w-full bg-gray-100 h-2 mt-5 overflow-hidden rounded-full">
              <div
          ...
          <div class="flex flex-col gap-3">
            <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest px-1 mb-1">
              Transactions
            </h3>
            ...
            @if (transactions().length > 0) {
              @for (tx of transactions(); track tx.id; let i = $index) {
                <button
                  ...
                  class="w-full bg-white border border-gray-100 rounded-2xl p-4 flex items-center justify-between gap-4 text-left shadow-sm hover:shadow-md active:scale-[0.99] transition-all"
                >
                  <div class="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div class="flex flex-col min-w-0">
                      ...
                      <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ tx.date | date: 'MMM d, yyyy' }}</span>
                    </div>
                  </div>
