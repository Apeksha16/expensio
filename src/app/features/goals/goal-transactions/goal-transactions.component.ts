    ...
    <div class="flex flex-col h-full relative">
      ...
      <div class="flex-1 overflow-y-auto p-4 pb-36">
        @if (goalService.isLoading() || expenseService.isLoading()) {
          ...
          <div class="mb-6 bg-black text-white p-6 shadow-xl relative overflow-hidden rounded-none border-2 border-black">
            ...
            <div class="w-full bg-white/20 h-2 mt-5 overflow-hidden rounded-none">
              <div class="h-full bg-white transition-all duration-500 origin-left animate-[fillProgress_1s_ease-out]" [style.width.%]="getProgress()"></div>
            </div>
          </div>

          <div class="flex flex-col gap-3">
            <h3 class="text-[11px] font-extrabold text-gray-500 uppercase tracking-widest px-1 mb-1">Transactions</h3>
            
            @if (transactions().length > 0) {
              @for (tx of transactions(); track tx.id; let i = $index) {
                <button (click)="editTransaction(tx)" class="w-full bg-white border-2 border-gray-100 p-4 flex items-center justify-between gap-4 text-left hover:bg-gray-50 active:bg-gray-100 transition-colors">
                  <div class="flex items-center gap-4 min-w-0">
                    <div class="w-10 h-10 bg-green-50 text-green-600 rounded-none border border-green-200 flex items-center justify-center shrink-0">
                      <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                      </svg>
                    </div>
                    <div class="flex flex-col min-w-0">
                      <span class="font-bold text-gray-900 truncate">{{ getTransactionName(i) }}</span>
                      <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest">{{ tx.date | date:'MMM d, y, h:mm a' }}</span>
                    </div>
                  </div>
                  <span class="font-extrabold text-green-600 shrink-0">+₹{{ tx.amount | number:'1.0-0' }}</span>
                </button>
              }
            } @else {
              <div class="flex flex-col items-center justify-center py-12 text-center">
                <div class="w-16 h-16 bg-gray-200 rounded-none border-2 border-gray-300 flex items-center justify-center mb-4">
