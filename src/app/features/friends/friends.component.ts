    ...
    <div class="h-full bg-white flex flex-col relative w-full overflow-hidden">
      ...
      <div class="px-4 pt-4 shrink-0 flex flex-col gap-4">
        @if (isInitialLoading()) {
          ...
          <div class="bg-gray-50 border border-gray-100 rounded-2xl h-[100px] animate-pulse"></div>
        } @else {
          @if (
            ...
            <div class="flex gap-4 w-full">
              <div class="flex-1 bg-[#F4F2FF] rounded-3xl p-5 flex flex-col items-start relative overflow-hidden shadow-sm">
                ...
                <span class="text-[11px] font-bold text-gray-500 mb-1">Total Friends</span>
                <span class="text-2xl font-extrabold text-gray-900 tracking-tight">
                  {{ friendService.acceptedFriends().length }}
                </span>
                <span class="text-[10px] font-bold text-gray-400 mt-1">connected to your account</span>
              </div>
            </div>
          }
        }
          ...
          @if (
                ...
                <div class="flex flex-col gap-3 mt-4">
                  ...
                  <div class="flex flex-col gap-3">
                    @for (req of friendService.incomingRequests(); track req.id) {
                      <div class="w-full bg-white border border-gray-100 rounded-3xl p-4 flex items-center justify-between gap-4 shadow-sm">
                        <div class="flex items-center gap-4">
                          <img
                            [src]="authService.getAvatarUrl(req.profile.avatarId)"
                            alt="Avatar"
                            class="w-12 h-12 rounded-full bg-gray-50 object-cover shrink-0"
                          />
                        ...
                        <div class="flex items-center gap-2">
                          <button
                            ...
                            class="w-10 h-10 flex items-center justify-center bg-gray-100 hover:bg-gray-200 transition-colors rounded-full active:scale-[0.98] disabled:opacity-50 text-gray-600"
                          >
                          ...
                          <button
                            ...
                            class="w-10 h-10 flex items-center justify-center bg-friends-primary hover:bg-friends-dark transition-colors rounded-full active:scale-[0.98] disabled:opacity-50 shadow-md shadow-friends-primary/30 text-white"
                          >
                ...
                <div class="flex flex-col gap-3 mt-4">
                  ...
                  <div class="flex flex-col gap-3">
                    @for (req of friendService.outgoingRequests(); track req.id) {
                      <div class="w-full bg-white border border-gray-100 rounded-3xl p-4 flex items-center justify-between gap-4 opacity-70 shadow-sm">
                        <div class="flex items-center gap-4">
                          <img
                            [src]="authService.getAvatarUrl(req.profile.avatarId)"
                            alt="Avatar"
                        ...
                        <button
                          ...
                          class="bg-gray-100 text-gray-500 px-3 py-1.5 rounded-full text-[11px] font-bold tracking-wide hover:bg-gray-200 transition-colors disabled:opacity-50 flex items-center gap-1"
                        >
                          @if (processingIds().has('remove_' + req.id)) {
                  ...
                  <div class="flex flex-col gap-3">
                    @for (friend of friendService.acceptedFriends(); track friend.id) {
                      <button
                        ...
                        class="w-full bg-white border border-gray-100 rounded-3xl p-4 flex items-center justify-between gap-4 text-left hover:shadow-md shadow-sm transition-all active:scale-[0.99]"
                      >
                        <div class="flex items-center gap-4">
                          <img
            ...
            <div class="mt-8 flex flex-col items-center justify-center text-center px-4">
              ...
              <div class="w-40 h-40 bg-gray-50 rounded-full flex items-center justify-center mb-6 border-8 border-white shadow-sm overflow-hidden">
                <img src="assets/images/empty-friends.png" alt="No Friends" class="w-full h-full object-cover opacity-80" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" />
              ...
              <button
                (click)="friendService.openAddSheet()"
                class="w-[200px] bg-friends-primary hover:bg-friends-dark text-white py-4 rounded-[20px] font-bold text-[14px] tracking-wide shadow-lg shadow-friends-primary/30 active:scale-95 transition-all flex items-center justify-center gap-2"
              >
