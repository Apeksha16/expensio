  ...
  template: `
    <div class="h-full bg-gray-50 flex flex-col relative w-full overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto bg-gray-50 relative pb-20 p-4">
        <div
          class="bg-splits-primary text-white p-6 mb-4 flex flex-col gap-4 rounded-none shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]"
        >
            ...
            @for (split of groupExpenses(); track split.id) {
              ...
              >
                <div class="flex justify-between items-start gap-4">
                  <span class="font-extrabold text-lg text-black truncate flex-1">{{
                    split.title
                  }}</span>
                ...
                <div
                  class="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest min-w-0 mt-1"
                >
                  <span class="whitespace-nowrap flex-shrink-0">{{
                    split.date | date: 'mediumDate'
                  }}</span>
                </div>
                        ...
                        @if (bal.pending) {
                          @if (bal.type === 'owed') {
                            <button
                              ...
                              [disabled]="
                                processingIds().has('confirm_' + split.id + '_' + bal.participantId)
                              "
