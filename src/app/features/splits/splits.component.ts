        ...
        >
          <div class="flex justify-between relative z-10 gap-4">
            <div class="flex flex-col flex-1 min-w-0">
              <span
                class="text-[10px] font-bold text-splits-surface uppercase tracking-widest mb-1 opacity-80"
                >You Are Owed</span
              >
                ...
                >
                  <div class="flex justify-between items-start gap-4">
                    <div class="flex flex-col gap-0.5 flex-1 min-w-0 pr-4">
                      <span class="font-extrabold text-lg text-splits-dark truncate">{{
                        split.title
                      }}</span>
                      <div
                        class="flex items-center gap-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest min-w-0"
                      >
                        <span class="truncate">{{ split.date | date: 'mediumDate' }}</span>
                      </div>
                    </div>
                    <span class="font-extrabold text-lg text-black flex-shrink-0"
                      >₹{{ split.total_amount | number: '1.0-2' }}</span
                    >
                  </div>
                  <div class="flex flex-col mt-1 w-full gap-2">
                    @for (bal of getExpenseBalances(split); track bal.participantId) {
                      <div class="flex justify-between items-center w-full gap-2">
                        <span
                          class="text-[10px] font-bold text-gray-500 uppercase tracking-widest truncate min-w-0"
                        >
                          <span class="truncate">
                            {{
                        ...
                        <div class="flex items-center gap-3">
                          <span
                            class="text-[10px] font-extrabold uppercase tracking-widest whitespace-nowrap"
                            [ngClass]="bal.type === 'owed' ? 'text-green-500' : 'text-red-500'"
                          >
                          ...
                          @if (bal.pending) {
                            @if (bal.type === 'owed') {
                              <button
                                ...
                                [disabled]="
                                  processingIds().has(
                                    'confirm_' + split.id + '_' + bal.participantId
                                  )
                                "
                      ...
                      <div class="flex justify-between items-center w-full gap-2">
                        ...
                        >
                          <span class="truncate"
                            >Paid by
                            {{
                              split.payer_id === currentUser().id
                                ? 'Me'
                                : getFriendName(split.payer_id)
                            }}</span
                          >
                        </span>
                        <span
                          class="text-[9px] font-extrabold text-gray-400 uppercase tracking-widest"
                          >Settled</span
                        >
                      </div>
                    }
              ...
              @for (group of splitService.activeGroups(); track group.id) {
                ...
                >
                  ...
                  @if (getGroupBalance(group.id).net > 0) {
                    <div
                      class="flex items-center gap-4 mt-2 border-t-2 border-gray-300 pt-2 w-full"
                    >
                      <span
                        class="text-[10px] font-extrabold text-green-600 uppercase tracking-wider"
                        >You are owed ₹{{ getGroupBalance(group.id).net | number: '1.0-2' }}</span
                      >
                    </div>
                    ...
                    <div
                      class="flex items-center gap-4 mt-2 border-t-2 border-gray-300 pt-2 w-full"
                    >
                      <span class="text-[10px] font-extrabold text-red-500 uppercase tracking-wider"
                        >You owe ₹{{ 0 - getGroupBalance(group.id).net | number: '1.0-2' }}</span
                      >
                    </div>
                  } @else {
                    <div class="flex items-center mt-2 border-t-2 border-gray-300 pt-2 w-full">
                      <span
                        class="text-[10px] font-extrabold text-gray-500 uppercase tracking-wider"
                        >Settled up</span
                      >
                    </div>
                  }
            ...
            @if (splitService.archivedGroups().length > 0) {
              <div class="mt-6 mb-2">
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest"
                  >Archived Groups</span
                >
              </div>
              @for (group of splitService.archivedGroups(); track group.id) {
                <div
                  class="w-full bg-white border border-gray-200 rounded-none p-4 flex flex-col gap-1 opacity-60"
                >
                  <div class="flex justify-between items-center w-full">
                    <span class="font-extrabold text-base text-gray-500 truncate">{{
                      group.name
                    }}</span>
                    <button
                  ...
                  <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest"
                    >{{ group.members.length }} members · Archived</span
                  >
                </div>
              }
            }
