        ...
        @if (ledgerService.isLoading()) {
          <div class="flex flex-col gap-3">
            @for (i of [1, 2, 3]; track i) {
              <div
                class="w-full bg-gray-200 rounded-none h-20 animate-pulse border-2 border-black"
              ></div>
            }
          </div>
        } @else {
          @if (ledger()) {
            ...
            @if (ledgerBalance() === 0) {
              <div
                class="mb-4 bg-green-200 border-2 border-black rounded-none p-4 flex items-center gap-4"
              >
                <div
                  class="w-10 h-10 bg-green-500 border-2 border-black text-black flex items-center justify-center rounded-none shrink-0"
                >
                ...
                <div>
                  <p class="text-sm font-extrabold text-black tracking-widest uppercase">
                    All Settled
                  </p>
            ...
            <!-- Summary Card -->
            <div
              class="mb-5 bg-ledger-primary p-4 rounded-none relative overflow-hidden text-white"
            >
              <div class="relative z-10 flex flex-col items-center text-center">
                <span
                  class="text-[10px] font-bold text-ledger-surface opacity-80 uppercase tracking-widest"
                ...
                <span
                  class="text-[10px] font-bold mt-1 uppercase tracking-widest text-ledger-surface opacity-90"
                >
                  @if (ledgerBalance() > 0) {
                    They still owe you
                ...
                <div
                  class="flex flex-col gap-0.5 w-full text-left bg-ledger-dark p-3 border border-ledger-dark/50 rounded-none"
                >
                  <span
                    class="text-[10px] font-bold text-ledger-surface opacity-80 uppercase tracking-widest"
                  >
                    ...
                    <span class="font-extrabold text-white"
                      >₹{{ $safeNavigationMigration(ledger()?.amount) | number: '1.0-0' }}</span
                    >
                    {{ ledger()?.type === 'in' ? 'Received' : 'Given' }}
                  </span>
                  @if (ledger()?.purpose) {
                    <span
                      class="text-[10px] font-medium text-ledger-surface opacity-60 uppercase tracking-wider"
                    >
                      {{ ledger()?.purpose }}
                    </span>
                  }
                ...
                @for (tx of transactions(); track tx.id) {
                  ...
                  >
                    <div class="flex flex-col gap-0.5 flex-1 min-w-0 pr-4">
                      <span class="font-extrabold text-lg text-black truncate">{{
                        tx.purpose || 'No note'
                      }}</span>
                      <div
            ...
            <div class="flex flex-col items-center justify-center py-12 text-center">
              <p class="text-gray-500 font-extrabold text-xl uppercase tracking-widest">
                Record not found
              </p>
              <button
                (click)="goBack()"
                class="mt-4 bg-white border-2 border-black text-black px-6 py-2.5 font-bold text-sm rounded-none uppercase tracking-widest hover:bg-black hover:text-white transition-colors"
              >
                Go Back
              </button>
            </div>
