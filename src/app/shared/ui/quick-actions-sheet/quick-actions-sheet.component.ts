...
@Component({
  ...
  template: `
    @if (quickActionsService.isSheetOpen()) {
      ...
      <div 
        @slideUp
        appSwipeToClose (swipeClose)="closeSheet()"
        class="fixed bottom-0 left-0 right-0 bg-black z-[70] max-h-[95vh] flex flex-col shadow-2xl"
      >
        <!-- Header -->
        <div class="flex justify-between items-center py-4 px-6 bg-black text-white sticky top-[-2px] z-10 shrink-0">
        ...
        <div class="p-6 flex flex-col gap-6 overflow-y-auto overscroll-none bg-white flex-1">
          <p class="text-[11px] font-semibold text-gray-500 tracking-widest uppercase">Select up to 4 items. Drag to reorder.</p>
          ...
          >
            @for (item of items(); track item.navItem.id) {
              ...
              >
                ...
                <div *cdkDragPreview class="flex items-center gap-3 p-3 bg-white border-2 border-[#1a2e22] shadow-2xl rounded-none select-none touch-manipulation box-border min-w-[300px]">
                  <div class="text-gray-400 p-1">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h16M4 16h16" />
                    </svg>
                  </div>
                  <div class="w-8 h-8 rounded-none border-2 border-black bg-gray-200 flex items-center justify-center text-black shrink-0">
                    <span [innerHTML]="item.navItem.icon" class="w-4 h-4 flex items-center justify-center"></span>
                  </div>
                  <div class="flex-1 font-bold text-sm text-gray-900 truncate">
                    {{ item.navItem.name }}
                  </div>
                  <input type="checkbox" [checked]="item.selected" class="w-5 h-5 accent-[#1a2e22] border-2 border-gray-300 rounded-none focus:ring-0 mr-2" readonly>
                </div>
...
export class QuickActionsSheetComponent {
  ...
  initializeItems() {
    ...
    const allModules = this.quickActionsService.navItems.filter(m => m.id !== 'dashboard' && m.id !== 'profile' && !m.hideFromBottomNav);
