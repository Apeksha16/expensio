...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, SwipeToCloseDirective, AmountInputDirective, AutofocusDirective, SafeInputDirective, DatePickerComponent, DayPickerComponent],
  animations: [
    trigger('slideUp', [
      transition(':enter', [
  ...
  template: `
    @if (goalService.isBottomSheetOpen()) {
      ...
      >
        ...
        >
          <h2 class="text-xl font-extrabold tracking-tight">{{ goalService.editingGoal()?.id ? 'Edit Goal' : 'Add Goal' }}</h2>
            ...
            <div class="flex justify-center mb-5">
              <span class="text-[9px] font-extrabold tracking-widest uppercase text-gray-400 bg-gray-100 px-3 py-1 rounded-none">
                @if (isUpdated()) {
                  ...
                  Added {{ goalService.editingGoal()?.created_at | date: 'medium' }}
                }
              </span>
            </div>
          }
          <form [formGroup]="goalForm" (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
            <div class="flex flex-col gap-1">
              ...
              <input
                ...
                class="w-full bg-white border-2 border-gray-200 text-gray-900 text-sm rounded-none focus:ring-0 focus:border-goals-primary hover:border-gray-300 block p-2.5 outline-none transition-all placeholder-gray-300 min-h-[44px] touch-manipulation font-sans"
              />
            </div>
            ...
            <div class="flex gap-4">
              <div class="flex-1 flex flex-col gap-1">
                ...
                <button
                  ...
                  class="w-full bg-white border-2 border-gray-200 text-gray-900 text-sm rounded-none focus:ring-0 focus:border-goals-primary hover:border-gray-300 block p-2.5 outline-none transition-all min-h-[44px] touch-manipulation font-sans flex justify-between items-center text-left"
                >
                  <span>{{ goalForm.get('target_date')?.value | date: 'MMM d, y' }}</span>
            ...
            <div class="flex flex-col gap-1">
              ...
              <div class="flex bg-gray-100 p-1">
                <button
                  ...
                  [ngClass]="goalForm.get('frequency')?.value === 'monthly' ? 'bg-black shadow-sm text-white' : 'text-gray-500 hover:bg-gray-200'"
                >
                  Monthly
                </button>
                <button
                  ...
                  class="flex-1 py-2 text-[11px] font-bold uppercase tracking-widest transition-colors duration-200"
                  [ngClass]="goalForm.get('frequency')?.value === 'alternate' ? 'bg-black shadow-sm text-white' : 'text-gray-500 hover:bg-gray-200'"
                >
                  Alternate
                </button>
                <button
                  ...
                  class="flex-1 py-2 text-[11px] font-bold uppercase tracking-widest transition-colors duration-200"
                  [ngClass]="goalForm.get('frequency')?.value === 'quarterly' ? 'bg-black shadow-sm text-white' : 'text-gray-500 hover:bg-gray-200'"
                >
                  Quarterly
                </button>
              </div>
    ...
    <app-date-picker
      ...
      [initialDate]="goalForm.get('target_date')?.value"
      (dateSelected)="onDateSelected($event)"
      (closed)="isDatePickerOpen = false"
    ></app-date-picker>
    <app-day-picker
      [isOpen]="isDayPickerOpen"
      [initialDay]="goalForm.get('installment_date')?.value"
...
export class GoalSheetComponent implements OnInit {
  ...
  goalIcons = [
    { name: 'Target', path: 'M15.042 21.672L13.684 16.6m0 0l-2.51 2.225.569-9.47 5.227 7.917-3.286-.672zm-7.518-.267A8.25 8.25 0 1120.25 10.5M8.288 14.212A5.25 5.25 0 1117.25 10.5' },
    ...
    { name: 'Vehicle', path: 'M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z' },
    ...
    { name: 'Tech', path: 'M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25m18 0A2.25 2.25 0 0018.75 3H5.25A2.25 2.25 0 003 5.25m18 0V12a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 12V5.25' },
    { name: 'Travel', path: 'M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5' },
    { name: 'Education', path: 'M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5' },
  ...
  constructor() {
    effect(() => {
      ...
      if (editing) {
        ...
        const currentTargetDate = this.goalForm?.get('target_date')?.value || this.getDefaultTargetDate();
        this.goalForm.reset({ 
          saved_amount: '0', 
  ...
  isUpdated(): boolean {
    ...
    const diff = Math.abs(new Date(goal.updated_at).getTime() - new Date(goal.created_at).getTime());
    return diff > 5000;
  }
