...
import { BottomSheetComponent } from '../../shared/ui/bottom-sheet/bottom-sheet';
import { ConfirmSheetComponent } from '../../shared/ui/confirm-sheet/confirm-sheet';
import { BudgetSheetComponent } from '../../shared/ui/budget-sheet/budget-sheet';
import { FriendSheetComponent } from '../../shared/ui/friend-sheet/friend-sheet';
import { SplitSheetComponent } from '../../shared/ui/split-sheet/split-sheet';
import { GroupSheetComponent } from '../../shared/ui/group-sheet/group-sheet';
...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-gray-50 flex flex-col relative w-full">
      <!-- Top Header -->
      <header class="fixed top-0 w-full bg-black z-30 h-14 flex items-center justify-between px-4 border-b-2 border-black">
        <ng-container *ngIf="isProfilePage(); else menuButton">
          <button (click)="goBack()" class="p-2 -ml-2 text-gray-400 hover:text-white focus:outline-none transition-colors">
        ...
        <ng-template #menuButton>
          <button (click)="toggleSidebar(true)" class="p-2 -ml-2 text-gray-400 hover:text-white focus:outline-none transition-colors">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        <div class="h-14 flex items-center px-6 border-b-2 border-black bg-gray-100">
          <span class="text-xl font-extrabold tracking-tight text-black">{{ authService.userProfile().username }}</span>
        </div>
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto overscroll-contain">
          <a *ngFor="let item of navItems" 
             ...
             <span [innerHTML]="item.icon" class="w-5 h-5" [ngClass]="{'text-current': true}"></span>
             {{ item.name }}
          </a>
        </nav>

        <div class="p-4 border-t-2 border-black">
          <button (click)="logout()" class="flex items-center justify-center gap-2 w-full p-3 font-extrabold text-white bg-red-600 border-2 border-red-600 hover:bg-red-700 hover:border-red-700 transition-colors rounded-none">
      ...
      <!-- Bottom Navbar -->
      <nav *ngIf="!isProfilePage()" class="fixed bottom-6 left-4 right-4 bg-white border-2 border-black z-30 rounded-none overflow-hidden">
        <div class="flex justify-between items-center h-16 w-full p-1.5 gap-1.5">
          <a *ngFor="let item of bottomNavItems" 
            ...
            <span [innerHTML]="item.icon" class="w-6 h-6 mb-1" [ngClass]="{'text-current': true}"></span>
      ...
      <button 
        *ngIf="!isProfilePage()"
        ...
        class="fixed bottom-[96px] right-4 w-14 h-14 bg-black text-white border-2 border-black rounded-none flex items-center justify-center z-40 hover:bg-white hover:text-black transition-colors">
        <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.5v15m7.5-7.5h-15" /></svg>
      </button>
...
export class Layout {
  ...
  navItems: { name: string, path: string, icon: any, hideFromBottomNav?: boolean }[] = [
    ...
    { 
      name: 'Budgets', 
      path: '/budgets', 
      icon: this.sanitizer.bypassSecurityTrustHtml('<svg fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6a7.5 7.5 0 107.5 7.5h-7.5V6z" /><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5H21A7.5 7.5 0 0013.5 3v7.5z" /></svg>')
    },
    { 
      name: 'Friends', 
      path: '/friends', 
