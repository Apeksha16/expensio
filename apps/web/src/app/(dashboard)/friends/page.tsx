    ...
    <div className="space-y-6 pb-6 select-none relative">
      ...
      <div className="flex items-center justify-between px-1">
        <div className="flex flex-col gap-0.5">
          <h2 className="text-xl font-extrabold tracking-tight text-zinc-100">Friends Balance</h2>
      ...
      <div className="grid grid-cols-2 gap-4 px-1">
        <div className="p-4 rounded-2xl border border-zinc-850 bg-zinc-900/20 backdrop-blur-xl flex flex-col gap-1.5 shadow-[0_4px_20px_rgba(0,0,0,0.1)] card-clean">
      ...
      <div className="px-1">
        <div className="flex bg-[#0c0d12]/60 border border-theme-border p-1 rounded-2xl justify-between items-center gap-1">
          <button
            onClick={() => setActiveTab('all')}
            className={`flex-1 py-2 px-1 flex items-center justify-center gap-1.5 text-center text-[10px] font-black uppercase tracking-wider rounded-xl transition-all duration-300 cursor-pointer outline-none ${
              activeTab === 'all'
                ? 'bg-zinc-900 border border-zinc-800 text-zinc-100 shadow-[0_4px_12px_rgba(0,0,0,0.4)] scale-[1.01]'
            ...
            <span
              className={`px-2 py-0.5 rounded-full text-[9px] font-black tracking-normal transition-colors ${
                activeTab === 'all'
                  ...
                  : 'bg-zinc-900/60 text-zinc-600 border border-zinc-850/40'
              }`}
            >
              {friends.length}
            </span>
          ...
          <button
            ...
            className={`flex-1 py-2 px-1 flex items-center justify-center gap-1.5 text-center text-[10px] font-black uppercase tracking-wider rounded-xl transition-all duration-300 cursor-pointer outline-none ${
              activeTab === 'requests'
                ? 'bg-zinc-900 border border-zinc-800 text-zinc-100 shadow-[0_4px_12px_rgba(0,0,0,0.4)] scale-[1.01]'
                : 'text-zinc-555 hover:text-zinc-300 border border-transparent'
            }`}
          >
            <span>Requests</span>
            <span
        ...
        {activeTab === 'requests' ? (
          requests.length === 0 ? (
            <div className="p-12 rounded-2xl border border-zinc-800/60 bg-zinc-900/20 backdrop-blur-md flex flex-col items-center justify-center gap-3.5 text-center">
              <div className="w-12 h-12 rounded-2xl bg-zinc-900 flex items-center justify-center text-zinc-600 border border-zinc-800/60">
            ...
            <div className="space-y-3">
              {requests.map((req) => (
                <div
                  key={req.id}
                  className="p-4 rounded-2xl border border-zinc-850/80 bg-zinc-900/20 backdrop-blur-xl flex items-center justify-between group card-clean"
                >
                  <div className="flex items-center gap-3.5">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <h4 className="text-sm font-bold text-zinc-100 leading-tight">{req.name}</h4>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 text-[9px] font-black uppercase active:scale-95 transition-all cursor-pointer"
                    >
                      Ignore
                    </button>
                  </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3.5 rounded-2xl bg-zinc-900/20 border border-zinc-850/60 focus:border-indigo-500/40 text-sm font-semibold text-zinc-100 placeholder-zinc-500 focus:outline-none transition-colors"
              />
            </div>
          </div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {searchedPresetUsers.map((user) => (
                  <div
                    ...
                    className="p-3.5 rounded-2xl border border-zinc-850 bg-zinc-900/20 hover:bg-zinc-900/45 cursor-pointer flex items-center justify-between group active:scale-98 transition-all card-clean"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg bg-zinc-950 border border-zinc-850 flex items-center justify-center font-bold text-xs text-zinc-400">
                        {user.avatar}
                      </div>
                      <div className="flex flex-col gap-0.5">
            ...
            <form onSubmit={handleAddCustomFriendSubmit} className="space-y-4">
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 rounded-2xl bg-zinc-900/20 border border-zinc-850/60 focus:border-indigo-500/40 text-xs font-semibold text-zinc-100 placeholder-zinc-650 focus:outline-none transition-colors"
                  required
                />
              </div>
          ...
          <div className="space-y-6 relative">
            {showSuccessOverlay && (
              <div className="absolute inset-0 z-50 bg-[#09090b]/95 flex flex-col items-center justify-center gap-3.5 text-center animate-fade-in">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-zinc-550">Balance is now fully squared up.</p>
                </div>
              </div>
            )}
            ...
            <div className="p-5 rounded-2xl border border-zinc-800/80 bg-zinc-900/40 backdrop-blur-xl relative overflow-hidden flex flex-col gap-5">
              ...
              <div className="flex flex-col items-center justify-center gap-1.5 py-2">
                ...
                <span
                  className={`text-4xl font-extrabold tracking-tight ${activeSettleFriend.balance > 0 ? 'text-cyan-400' : 'text-rose-400'}`}
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="py-4 rounded-xl bg-gradient-to-tr from-indigo-500 to-cyan-500 text-zinc-950 font-black text-xs uppercase tracking-widest hover:shadow-lg active:scale-98 transition-all cursor-pointer flex items-center justify-center gap-1.5 border border-indigo-300/20"
              >
