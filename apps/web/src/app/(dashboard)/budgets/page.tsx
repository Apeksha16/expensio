...
import { Plus, ChevronLeft, ChevronRight, PieChart, Trash2, AlertTriangle } from 'lucide-react';
...
export default function BudgetsPage() {
  ...
  const categories = ['Food', 'Shopping', 'Transport', 'Entertainment', 'Utilities', 'Bills', 'Health', 'Education'];
  ...
  return (
    <div className="space-y-6 pb-6 select-none relative">
      ...
      <div className="flex items-center justify-between px-1">
        <h2 className="text-xl font-bold tracking-tight text-zinc-100">Category Budgets</h2>
        ...
        <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-xl p-1 shrink-0">
          ...
          <span className="text-[10px] font-black uppercase tracking-wider text-zinc-300 px-1">
            May 2025
          </span>
          <button className="p-1 text-zinc-500 hover:text-zinc-300 active:scale-95 transition-all">
      ...
      <div className="p-5 rounded-3xl border border-zinc-850 bg-gradient-to-br from-zinc-900/60 to-zinc-950/80 backdrop-blur-xl space-y-4 px-5">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-black uppercase tracking-wider text-zinc-500">Overall Progress</span>
        ...
        {budgets.length === 0 ? (
          <div className="p-12 rounded-2xl border border-zinc-800/60 bg-zinc-900/20 backdrop-blur-md flex flex-col items-center justify-center gap-3 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-zinc-500 max-w-[240px]">Tap "New Budget" above to set spending limits for your categories.</p>
            </div>
          </div>
        ) : (
      ...
      >
        {availableCategories.length === 0 ? (
          <div className="text-center py-6 space-y-4">
            ...
            <div className="space-y-1.5 px-4">
              ...
              <p className="text-xs text-zinc-550 leading-relaxed">
                You have already created spending limits for all available spending categories. Delete or edit an existing budget to adjust.
              </p>
            </div>
            <button
              type="button"
          ...
          <form onSubmit={handleAddBudgetSubmit} className="space-y-5">
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Select Category</label>
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value)}
                className="w-full px-3 py-3.5 rounded-xl bg-zinc-900/60 border border-zinc-800 text-xs font-semibold text-zinc-100 focus:outline-none"
              >
      ...
      >
        {editingBudget && (
          <form onSubmit={handleEditBudgetSubmit} className="space-y-5">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Budget Category</span>
              <span className="text-sm font-extrabold text-zinc-300 bg-zinc-950/60 border border-zinc-900 px-3.5 py-3 rounded-xl block">
            ...
            {/* Premium contextual delete trigger inside editing drawer */}
            <div className="border-t border-zinc-850 pt-4 mt-2">
              <button
                ...
                className="w-full py-3.5 rounded-xl border border-rose-500/20 bg-rose-550/5 text-rose-400 hover:bg-rose-500/10 hover:border-rose-500/30 font-bold active:scale-98 transition-all text-xs cursor-pointer flex items-center justify-center gap-1.5 transition-colors duration-200"
              >
          ...
          <div className="space-y-5">
            <div className="text-center py-2 space-y-3">
              ...
              <div className="space-y-1.5 px-2">
                <h4 className="text-sm font-black text-zinc-200">Delete {deletingBudget.category} Budget?</h4>
                <p className="text-xs text-zinc-500 leading-relaxed max-w-xs mx-auto">
                  Are you sure you want to permanently remove the limit for <span className="text-zinc-350 font-bold">{deletingBudget.category}</span>? Your actual expenses in this category will remain untouched.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="py-4 rounded-xl bg-gradient-to-tr from-rose-500 to-rose-600 text-zinc-950 font-bold hover:shadow-lg active:scale-98 transition-all cursor-pointer text-xs uppercase tracking-wider"
              >
                Yes, Delete
              </button>
              <button
