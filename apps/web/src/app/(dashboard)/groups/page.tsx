      ...
      {activeGroup ? (
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <div
            ...
            style={
              activeGroup.coverUrl ? { backgroundImage: `url(${activeGroup.coverUrl})` } : undefined
            }
          >
            ...
            <span className="text-[9px] font-black uppercase tracking-widest text-zinc-200 z-10">
              Group Split
            </span>
            <h3 className="text-2xl font-black text-zinc-100 tracking-tight z-10 leading-none">
              {activeGroup.name}
            </h3>
            <p className="text-[11px] text-zinc-300 line-clamp-2 leading-relaxed z-10 opacity-90 max-w-xs font-semibold">
          ...
          <div className="grid grid-cols-2 gap-4 px-0.5">
            <div className="p-4 rounded-2xl border border-zinc-800 bg-zinc-900/20 backdrop-blur-md flex flex-col gap-1 shadow-sm card-clean">
              ...
              <span className="text-xl font-extrabold text-zinc-100">
                ₹{activeGroupTotalSpend.toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </span>
            </div>
            ...
            <div className="p-4 rounded-2xl border border-zinc-800 bg-zinc-900/20 backdrop-blur-md flex flex-col gap-1 shadow-sm card-clean">
              ...
              <span className="text-xl font-extrabold text-indigo-400">
                ₹
                {(activeGroupTotalSpend / (activeGroup.members.length + 1)).toLocaleString(
                  'en-US',
          ...
          <div className="space-y-3 px-1">
            {activeGroupExpenses.length === 0 ? (
              <div className="p-10 rounded-2xl border border-zinc-850 bg-zinc-950/20 backdrop-blur-md flex flex-col items-center justify-center gap-3 text-center py-10 px-4 card-clean">
                ...
                <div className="space-y-0.5">
                  ...
                  <p className="text-[10px] text-zinc-550">
                    Expenses split equally in this group will appear here.
                  </p>
                </div>
              </div>
            ) : (
          ...
          <div className="space-y-3 px-1">
            ...
            <div className="p-4 rounded-2xl border border-zinc-800 bg-zinc-900/20 backdrop-blur-md flex flex-wrap gap-2.5 card-clean">
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl border border-indigo-500/20 bg-indigo-500/10 text-indigo-400 text-xs font-bold shadow-sm">
              ...
              {activeGroup.members.map((member: string) => (
                <div
                  key={member}
                  className="flex items-center gap-2 px-3 py-2 rounded-xl border border-zinc-850 bg-zinc-950/40 text-zinc-350 text-xs font-bold shadow-sm"
                >
                  <div className="w-4.5 h-4.5 rounded bg-zinc-900 border border-zinc-850 flex items-center justify-center text-[9px] font-black uppercase text-zinc-500">
                    {member
                      .split(' ')
                      .map((n) => n[0])
                      .join('')}
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex flex-col gap-0.5">
              ...
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">
                Manage shared trip or flat expenses
              </p>
            </div>
            <button
            ...
            {groups.length === 0 ? (
              <div className="p-12 rounded-2xl border border-zinc-800/60 bg-zinc-900/20 backdrop-blur-md flex flex-col items-center justify-center gap-3 text-center">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-bold text-zinc-350">No active split groups</p>
                  <p className="text-xs text-zinc-550">
                    Tap the + button to create a shared group expense space.
                  </p>
                </div>
              </div>
            ) : (
        ...
        <form onSubmit={handleAddGroupSubmit} className="space-y-6">
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className="w-full px-4 py-3.5 rounded-2xl bg-zinc-900/20 border border-zinc-850/60 focus:border-indigo-500/40 text-sm font-semibold text-zinc-100 placeholder-zinc-500 focus:outline-none transition-colors"
              required
            />
          </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2.5 px-4 py-2.5 rounded-2xl border font-bold text-xs transition-all shrink-0 cursor-pointer ${
                        isChecked
                          ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/35 ring-1 ring-indigo-500/30 shadow-md'
        ...
        <form onSubmit={handleAddGroupExpenseSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className="w-full px-4 py-3.5 rounded-2xl bg-zinc-900/20 border border-zinc-850/60 focus:border-indigo-500/40 text-sm font-semibold text-zinc-100 focus:outline-none placeholder-zinc-500"
              required
            />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className="w-full px-4 py-3.5 rounded-2xl bg-zinc-900/20 border border-zinc-850/60 focus:border-indigo-500/40 text-sm font-semibold text-zinc-100 focus:outline-none placeholder-zinc-550"
            />
          </div>

          {activeGroup && (
            <div className="p-3.5 rounded-2xl border border-zinc-850 bg-[#0c0d12]/60 flex items-center justify-between text-xs text-zinc-500 font-semibold shadow-sm card-clean">
              <div className="flex items-center gap-1.5">
                <Share2 className="w-4 h-4 text-indigo-400" />
                <span>Split Equally Between:</span>
              </div>
              <span className="text-zinc-350">{activeGroup.members.length + 1} Members</span>
            </div>
          )}
