...
const categoryMeta: Record<string, { icon: React.ComponentType<any>; color: string; bg: string }> =
  {
    Food: { icon: Coffee, color: 'text-indigo-400', bg: 'bg-indigo-500/10 border-indigo-500/20' },
    Shopping: {
      icon: ShoppingBag,
      color: 'text-pink-400',
...
export default function BudgetCard({ budget, onDelete, onEdit }: BudgetCardProps) {
  ...
  return (
    <div className="p-4.5 rounded-[22px] border border-white/[0.04] bg-[#0c0d12]/30 shadow-[0_8px_20px_rgba(0,0,0,0.15)] relative overflow-hidden group flex flex-col gap-4.5 transition-all duration-200 card-clean hover:border-white/[0.08]">
      {/* Category header row */}
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            <h4 className="text-xs font-bold text-zinc-100">{budget.category}</h4>
            <div className="flex items-baseline gap-1 text-[10px] text-zinc-550 font-semibold mt-0.5">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isExceeded && !isWarning && (
            <span className="flex items-center gap-0.5 text-[8px] font-black uppercase tracking-wider text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-lg border border-emerald-500/20">
              <Star className="w-2.5 h-2.5 fill-emerald-400 text-emerald-400" />
              <span>Good</span>
            </span>
          )}
          {isWarning && (
            <span className="text-[8px] font-black uppercase tracking-wider text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-lg border border-amber-500/20">
              Warning
            </span>
          )}
          {isExceeded && (
            <span className="text-[8px] font-black uppercase tracking-wider text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-lg border border-rose-500/20">
              Over Limit
            </span>
          )}
          ...
          {onEdit && (
            <button
              ...
              className="p-1.5 rounded-lg bg-zinc-950/40 border border-zinc-850 text-zinc-400 hover:text-indigo-400 hover:border-indigo-500/25 active:scale-95 transition-all cursor-pointer flex items-center justify-center shrink-0"
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-zinc-950/40 border border-zinc-850 text-zinc-400 hover:text-rose-400 hover:border-rose-500/25 active:scale-95 transition-all cursor-pointer flex items-center justify-center shrink-0"
        ...
        <div className="h-1.5 w-full bg-zinc-950 rounded-full p-[1px] border border-zinc-850 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-700 ${
              isExceeded ? 'bg-rose-500' : isWarning ? 'bg-amber-500' : 'bg-indigo-400'
            }`}
            style={{ width: `${Math.min(100, percent)}%` }}
          />
        </div>

        <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-wider text-zinc-550">
