  ...
  return (
    <div className="p-4 rounded-2xl border border-zinc-850 bg-zinc-900/20 backdrop-blur-md flex items-center justify-between group active:scale-[0.99] transition-all shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:border-zinc-800">
      <div className="flex items-center gap-3.5">
        ...
        <div className={`w-11 h-11 rounded-xl flex items-center justify-center border font-bold text-xs uppercase tracking-wider ${
          isOwed 
        ...
        <div className="flex flex-col items-end gap-0.5">
          {isOwed ? (
            <>
              ...
              <span className="text-sm font-black text-cyan-400 tracking-tight">₹{friend.balance.toFixed(2)}</span>
            </>
          ) : isOwing ? (
            <>
              <div className="flex items-center gap-0.5 text-[8px] text-zinc-500 font-bold uppercase tracking-widest">
              ...
              <span className="text-sm font-black text-rose-400 tracking-tight">₹{Math.abs(friend.balance).toFixed(2)}</span>
            </>
          ) : (
